export default {
  async loadBooks(context, payload) {
    ...
    const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${payload?.query}`)
  ...
  async loadBook(context, payload) {
    ...
    const response = await fetch(`https://www.googleapis.com/books/v1/volumes/${payload?.id}`)
    const responseData = await response.json()
